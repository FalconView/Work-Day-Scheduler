...
$("#curntDay").text(moment().format("dddd, MMMM Do"));
...
var createTimeblocks = function (start, end) {
  ...
  for (var i = start; i < end + 1; i++) {
    ...
    // create a container
    var timeBlockEl = $("<div>").addClass("block-container row");
    ...
    var eventEl = $("<p>")
      ...
      .attr("id", "event");
    var saveBtn = $("<button>").addClass("saveBtn col-2 col-md-1");
    var iconEl = $("<i>").addClass("ri-save-2-fill");
    ...
    $("#timebloks-container").append(timeBlockEl);

    // get element's content to store in events array
    var dateEl = $("#curntDay").text().trim();
...
$("#timebloks-container").on("click", "p", function () {
...
$("#timebloks-container").on("blur", "textarea", function () {
...
var checkCurrentTime = function (time, eventEl) {
  ...
  var timeblockTime = moment(today + " " + time, dateFormat).format(
    "YYYY-MM-DD kk:mm"
  );
...
$("#timebloks-container").on("click", "button", function () {
...
setInterval(function () {
  $(".description").each(function (index) {
    ...
    var eventEl = $(".hour[datetime$='" + time + "']")
      .parent()
      .find("p");
